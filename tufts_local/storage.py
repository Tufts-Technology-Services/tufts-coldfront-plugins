...
from coldfront_utils import bytes_to_units, update_allocation_attribute_value
...
from storage.starfish import get_path_usage_data, get_starfish_usage_data_by_volume, validate_starfish_path
...
def get_quotas_batch(resource_id=None, client_config_id=None):
    ...
    for alloc in resource_allocations:
        if not alloc.allocationattribute_set.filter(allocation_attribute_type__name=path_attr).exists():
            logger.warning(
                f"Allocation {alloc.pk} of resource {resource.name} is missing required native path attribute '{path_attr}' and will be skipped in Starfish usage retrieval task."
            )
        else:
            ...
            native_path_value = (
                alloc.allocationattribute_set.filter(allocation_attribute_type__name=path_attr).first().value
            )
            try:
                validate_starfish_path(native_path_value)
            except ValueError as e:
                logger.warning(
                    f"Allocation {alloc.pk} of resource {resource.name} has native path attribute value '{native_path_value}' that is invalid: {e}. This allocation will be skipped in Starfish usage retrieval task."
                )

    # now filter to only include allocations with valid native path attributes
    sf_attrs = AllocationAttribute.objects.filter(
        allocation__in=resource_allocations, allocation_attribute_type__name=path_attr
    )
    ...
    for vol in volumes:
        ...
        for vol_path in vol_attributes:
            try:
                ...
                logger.warning(
                    f"Skipping allocation {vol_path.allocation.pk} with invalid Starfish path '{vol_path.value}': {e}"
                )
                continue
            usage, report_date = get_path_usage_data(volume_data, vol_path.value)
            if usage and report_date:
                logger.info(
                    f'Updating usage for allocation {vol_path.allocation.pk} with usage {usage} bytes and report date {report_date}'
                )
                ...
                update_allocation_attribute_value(vol_path.allocation, QUOTA_IN_BYTES_ATTRIBUTE_NAME, usage)
                update_allocation_attribute_value(
                    vol_path.allocation,
                    QUOTA_ATTRIBUTE_NAME,
                    str(round(bytes_to_units(usage, STORAGE_PLUGIN_STORAGE_UNITS), 2)),
                )
                update_allocation_attribute_value(
                    vol_path.allocation, QUOTA_REPORT_DATE_ATTRIBUTE_NAME, report_date.isoformat()
                )
            else:
                logger.warning(
                    f'No matching subfolder found for allocation attribute with vol_path value {vol_path.value}'
                )
    return True
