...
def sf_report(request):
    ...
    vol_path_allocation_attributes = set(
        list(
            AllocationAttribute.objects.filter(
                allocation__in=storage_allocations, allocation_attribute_type__name='sf_vol_path'
    ...
    if request.GET.get('format') == 'csv':
        data = {'header': ['sf_status', 'sf_vol_path', 'Resource Allocation', 'Project', 'Status'], 'rows': []}
        for allocation in missing_sf_attribute:
            sf_vol_path = allocation.allocationattribute_set.filter(
                allocation_attribute_type__name='sf_vol_path'
            ).first()
        ...
        for vol_path in missing_from_coldfront:
            data['rows'].append(['In Starfish but not in Coldfront', vol_path, '', '', ''])
...
def not_updated_report(request):
    """
    View for the not updated report. Shows storage allocations not updated with latest usage and quota information.
    Used to identify allocations that need attention.
    todo: move to storage plugin
    """
